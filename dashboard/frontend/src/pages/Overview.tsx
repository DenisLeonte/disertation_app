    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {status && trajectory && (
        <dl style={{ display: 'flex', gap: 32, margin: 0 }}>
          ...
          <Num label="best loss" value={status.best_ever?.toFixed(6) ?? '--'} green />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <Section title="loss trajectory">
          {trajectory ? <LossTrajectory data={trajectory.points} bestEver={trajectory.best_ever} /> : <Skel h={240} />}
        </Section>
        <Section title="train / val spread">
          {log ? <TrainVal rows={log} /> : <Skel h={240} />}
        </Section>
      </div>

      {/* architecture + diversity row */}
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
        <Section title="diversity per generation">
          {diversity ? <DiversityBars data={diversity.per_generation} /> : <Skel h={180} />}
        </Section>
        <Section title={`population gen ${latest?.[0]?.generation ?? '?'}`}>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h3 className="dim" style={{ fontSize: 11, fontWeight: 500, margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{title}</h3>
      {children}
    </section>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <AreaChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey="generation" stroke="#333" fontSize={10} tickLine={false} axisLine={false} />
        <YAxis stroke="#333" fontSize={10} tickLine={false} axisLine={false} width={48} tickFormatter={(v: number) => v.toFixed(3)} />
        <Tooltip {...tt} labelFormatter={(g) => `gen ${g}`} />
        <Area type="monotone" dataKey="gen_best_val" stroke="#555" strokeWidth={1} fill="none" dot={false} name="gen best" />
        <Area type="monotone" dataKey="best_so_far" stroke="var(--green)" strokeWidth={1.5} fill="url(#lg)" dot={false} name="best ever" />
        <ReferenceLine y={bestEver} stroke="var(--green)" strokeDasharray="3 3" strokeOpacity={0.4} />
      </AreaChart>
    </ResponsiveContainer>
  );
}
...
function TrainVal({ rows }: { rows: LogRow[] }) {
  const data = useMemo(() => {
    ...
    return Array.from(byGen.entries()).map(([gen, pop]) => ({
      gen,
      train: pop.reduce((s, r) => s + (r.train_loss ?? 0), 0) / pop.length,
      val: pop.reduce((s, r) => s + r.val_loss, 0) / pop.length,
      best: Math.min(...pop.map(r => r.val_loss)),
    }));
  }, [rows]);
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
        <XAxis dataKey="gen" stroke="#333" fontSize={10} tickLine={false} axisLine={false} />
        <YAxis stroke="#333" fontSize={10} tickLine={false} axisLine={false} width={48} tickFormatter={(v: number) => v.toFixed(3)} />
        <Tooltip {...tt} labelFormatter={(g) => `gen ${g}`} />
        <Line type="monotone" dataKey="train" stroke="var(--yellow)" strokeWidth={1} dot={false} name="avg train" />
        <Line type="monotone" dataKey="val" stroke="var(--red)" strokeWidth={1} dot={false} name="avg val" />
        <Line type="monotone" dataKey="best" stroke="var(--green)" strokeWidth={1.5} dot={false} name="best val" />
      </LineChart>
    </ResponsiveContainer>
  );
}

function DiversityBars({ data }: { data: DiversityGen[] }) {
  return (
    <ResponsiveContainer width="100%" height={180}>
      <BarChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
        <XAxis dataKey="generation" stroke="#333" fontSize={10} tickLine={false} axisLine={false} />
        <YAxis stroke="#333" fontSize={10} tickLine={false} axisLine={false} width={28} />
        <Tooltip {...tt} labelFormatter={(g) => `gen ${g}`} />
        <Bar dataKey="unique_archs" name="unique archs" radius={[2, 2, 0, 0]} maxBarSize={12}>
          {data.map((d, i) => <Cell key={i} fill={d.is_monoculture ? 'var(--red)' : '#555'} />)}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      {counts.map(({ type, count, pct }) => (
        <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span className="mono dim" style={{ width: 80, fontSize: 11 }}>{type.replace('_', ' ')}</span>
          <div style={{ flex: 1, height: 6, background: 'var(--bg-inset)', borderRadius: 1 }}>
            <div style={{ width: `${pct * 100}%`, height: '100%', background: '#555', borderRadius: 1 }} />
          </div>
          <span className="mono dim" style={{ width: 24, textAlign: 'right', fontSize: 11 }}>{count}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <section>
      <h3 className="dim" style={{ fontSize: 11, fontWeight: 500, margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
        individuals — gen {sorted[0]?.generation}
      </h3>
        ...
        <table style={{ width: '100%', fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)' }}>
              ...
              <th className="dim" style={{ padding: '4px 8px', fontSize: 11 }}>val loss</th>
              <th className="dim" style={{ padding: '4px 8px', fontSize: 11 }}>train loss</th>
